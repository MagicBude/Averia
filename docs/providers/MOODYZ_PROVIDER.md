...
如果 Transport 已成功建立请求，但来源站、CDN 或代理链路临时返回以下状态码：

```text
408 / 429 / 500 / 502 / 503 / 504
```

网络层会默认最多尝试 3 次，并按指数退避等待：

```text
第 1 次失败 → 等待约 750ms
第 2 次失败 → 等待约 1500ms
第 3 次仍失败 → 返回最后一次 HTTP 状态并停止
```

`404` 等永久错误不会重试。成功时终端会输出 `网络尝试：N 次`，`meta.json` 记录 `network_attempts`。该机制仅用于吸收正常公开站点的瞬时网络/CDN错误，不用于绕过访问控制。
